...
export default class Game extends Phaser.Scene {
    preload() {
        // 1. Find out which unit the player is currently playing
        const selectedUnit = localStorage.getItem('selectedChapter') || 'Unit 4.5';
        
        // 2. Map the unit to the correct badge image path from your HTML
        let badgePath = '';
        switch(selectedUnit) {
            case 'Unit 4.5': badgePath = './web_assets/badge_unit_4_5.png'; break;
            case 'Unit 6.1': badgePath = './web_assets/badge_unit_6_1.png'; break;
            case 'Unit 6.2': badgePath = './web_assets/badge_unit_6_2.png'; break;
            case 'Unit 6.3': badgePath = './web_assets/badge_unit_6_3.png'; break;
            case 'Unit 6.4': badgePath = './web_assets/badge_unit_6_4.png'; break;
            case 'Unit 15.1_15.2': badgePath = './web_assets/badge_unit_15.png'; break;
            case 'All': badgePath = './web_assets/badge_unit_all.png'; break;
            default: badgePath = './web_assets/badge_unit_4_5.png'; 
        }
    ...
    // Filter questions based on the selected unit (or 'All' for everything)
    let filteredQuestions;
    if (selectedUnit === 'All') {
        filteredQuestions = questionData; 
    } else if (selectedUnit === 'Unit 15.1_15.2') {
        // Your JSON uses "Chapter": 15 for the reproductive system
        filteredQuestions = questionData.filter(q => q.Chapter === 15);
    } else {
        // Extract the number (e.g., 4.5 from 'Unit 4.5') and match to 'Subtopic'
        const targetSubtopic = parseFloat(selectedUnit.replace('Unit ', ''));
        filteredQuestions = questionData.filter(q => q.Subtopic === targetSubtopic);
    }
    ...
    this.pools = {
        1: filteredQuestions.filter(q => q.Level === 1),
        2: filteredQuestions.filter(q => q.Level === 2),
        3: filteredQuestions.filter(q => q.Level === 3)
    };
   ...
   hitPortal(player, portal) {
        ...
        // 2. Stop the player and make them immune to gravity
        this.player.setVelocityX(0);
        ...
        // Create a burst of 20 tiny DNA coins or sparks shooting outwards
        const particles = this.add.particles(portal.x, portal.y, 'dna_coin', {
            speed: { min: -150, max: 150 },
            angle: { min: 0, max: 360 },
            scale: { start: 0.1, end: 0 }, // Shrink to nothing
            lifespan: 800,
            emitting: false // Don't loop, we just want one burst
        });
        particles.explode(20); // Fire 20 particles instantly

        // 3. The "Portal Suck" Animation
        this.tweens.add({
            targets: this.player,
            x: portal.x,          // Move to center of portal
            y: portal.y - 20,     // Adjust slightly based on your portal sprite anchor
            scale: 0,             // Shrink into nothingness
            angle: 360,           // Spin 360 degrees
            duration: 800,        // Takes 0.8 seconds
            ease: 'Power2',
            onComplete: () => {
                // 4. Fade to black after they get sucked in
                this.cameras.main.fadeOut(500, 0, 0, 0);
                
                // 5. Wait for the fade to finish, THEN load the next level
                this.cameras.main.once(Phaser.Cameras.Scene2D.Events.FADE_OUT_COMPLETE, () => {
   ...
   handleLevelComplete() {
        ...
        if (this.currentLevel > 3) {
            ...
            // Re-enable camera and player size so the final cutscene is visible
            ...
            this.tweens.add({
                ...
                ease: 'Power2',
                onComplete: () => {
                    const speech = this.add.text(sage.x, sage.y - 140, "You have finally reached me.\nKnowledge is your greatest shield.", {
                        fontSize: '20px', fontFamily: '"Poppins", sans-serif', fill: '#ffffff',
                        align: 'center', backgroundColor: '#1a3b22', padding: { x: 15, y: 10 }
                    }).setOrigin(0.5);

                    this.time.delayedCall(2500, () => {
                        // 1. Hide the speech bubble
                        speech.destroy(); 

                        // 2. Spawn the badge small and invisible at the Sage's staff/hand
                        // Check which chapter is playing to select the right badge key
                        const currentChapter = localStorage.getItem('selectedChapter') || 'Unit 4.5';
                        const badgeKey = 'badge_' + currentChapter;

                        // Spawn the correct preloaded badge
                        const badge = this.add.image(sage.x - 20, sage.y - 20, badgeKey)
                            .setScale(0)
                            .setAlpha(0)
                            .setDepth(50); // Ensure it renders on top

                        // 3. Animate it floating up, spinning, and growing to the center!
                        this.tweens.add({
                            targets: badge,
                            scale: 0.2,              // Grow to a nice big size
                            alpha: 1,                // Fade in
                            x: this.cameras.main.midPoint.x, // Move to center screen
                            y: this.cameras.main.midPoint.y - 50,
                            angle: 360,              // Spin once dynamically
                            duration: 1500,          // Takes 1.5 seconds
                            ease: 'Back.easeOut',    // Gives it a satisfying "pop" into place
                            onComplete: () => {
                                // 4. Wait 1 second for the player to admire it, then show victory screen
        ...
        if (this.hearts < this.maxHearts) {
            this.hearts += 0.5; // Heal half a heart
            // Clamp to max hearts so they don't over-heal
            if (this.hearts > this.maxHearts) {
                this.hearts = this.maxHearts;
            }
            this.updateHeartsUI(); // Refresh the HUD icons
        }
        
        // Reset Player Position & Physics BEFORE spawning UI text
        this.player.setPosition(100, 300);
        this.player.setScale(0.3);         // Reset to original size
        this.player.setAngle(0);   
        this.player.body.enable = true;    // Turn physics back on 
        this.player.body.setAllowGravity(true); // Turn gravity back on
        this.player.clearTint();           // Just in case they had damage tint
        this.cameras.main.fadeIn(500);

        // Create the giant level text perfectly centered on the camera
        const bigLevelText = this.add.text(500, 250, 'LEVEL ' + this.currentLevel, {
            fontSize: '72px',
            fontFamily: 'Georgia, serif', // Matches your game's fantasy theme
            fill: '#FFD700', // Gold color
            fontWeight: 'bold',
            stroke: '#362f29',
            strokeThickness: 10,
            shadow: { offsetX: 0, offsetY: 5, color: '#000', blur: 5, stroke: true, fill: true }
        }).setOrigin(0.5).setScrollFactor(0).setDepth(100); 

        // Animate the text to scale up and fade out
        this.tweens.add({
            targets: bigLevelText,
            scale: 1.5,       
            alpha: 0,         
            duration: 2000,   
            ease: 'Power2',
            onComplete: () => {
                bigLevelText.destroy(); 
            }
        });

        // Calculate Bonus: "Flawless Scholar" (Moved here to prevent camera snap glitches)
        if (this.levelMistakes === 0) {
            ...
            }).setOrigin(0.5).setScrollFactor(0).setDepth(101); // Renders above the big Level text

            // Delay by 500ms so it floats up right as the giant level text is fading
            this.tweens.add({ targets: bonusText, y: bonusText.y - 50, alpha: 0, duration: 2000, delay: 500 });
        }

        // Reset level mistakes for the next stage
        this.levelMistakes = 0;

        // Spawn next level entities
    ...
    displayFinalVictory() {
        ...
        homeBtn.on('pointerdown', () => {
            ...
            // Stop the music before navigating away
            if (this.music && this.music.isPlaying) {
                this.music.stop();
            }

            // Redirect back to the selection page
            window.location.href = 'game_chose_page.html'; 
        });
    }
